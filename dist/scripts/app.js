...
$templateCache.put('app/directives/list-item/list-item.html','<md-input-container style="width: 100%" flex=""><label>{{ \'DATA_SRC\' | translate }}</label><md-select ng-model="List.item.dataSrcSelected"><md-option><em>{{\'NONE\' | translate}}</em></md-option><md-option ng-repeat="dtSrc in List.sources" ng-value="dtSrc">{{dtSrc.description}}</md-option></md-select></md-input-container>');
$templateCache.put('app/directives/list-item/list-view.html','<md-input-container style="width: 100%;"><md-select ng-model="List.item.dataSrcSelected" ng-disabled="true" placeholder="{{ListView.formItem.dataSrcSelected.description}}"></md-select></md-input-container>');
